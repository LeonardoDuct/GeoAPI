...
Aqui, estamos criando a classe e o GeocodingAPImétodo getLocation(), que irá retornar a localização geográfica do endereço fornecido. A constante API_KEYé usada para armazenar sua chave de API.

Java

        String urlString = "http://www.mapquestapi.com/geocoding/v1/address?key=" + API_KEY + "&location=" + address;
        URL url = new URL(urlString);
Aqui, estamos criando uma string de URL para a API de geocodificação, incluindo sua chave de API e o endereço que desejamos geocodificar. Em seguida, criamos uma URL a partir dessa string.
...
Aqui, estamos abrindo uma conexão HTTP com a API de geocodificação e definindo o método de solicitação como GET. Estamos também definindo o tipo de conteúdo da solicitação como JSON.
